...
//Topik praktikum	: Praktikum 8-set.c
...
Set SetIntersection(Set s1, Set s2){
	...
	int i;
	for (i=0;i<s1.Count;i++){
		if ((IsMember(s1,s1.Elements[i])) && (IsMember(s2,s1.Elements[i]))){
			Insert(&s3,s1.Elements[i]);
			}
		}
	return s3;
...
Set SetSymmetricDifference(Set s1, Set s2){
	...
	for (i=0;i<s2.Count;i++){
		if ((!(IsMember(s1,s2.Elements[i])) && (IsMember(s2,s2.Elements[i])))){
			Insert(&s3,s2.Elements[i]);
		}
	}
	return s3;
...
Set SetSubtract(Set s1, Set s2){
	...
	for (i=0;i<s2.Count;i++){
		if ((IsMember(s3,s2.Elements[i])) && (IsMember(s2,s2.Elements[i]))){
			Delete(&s3,s2.Elements[i]);
			}
		}
	return s3;
	}
